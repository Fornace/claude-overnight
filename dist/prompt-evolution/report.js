...
export function generateReport(opts, result, generationMatrices, testMatrix) {
    ...
    if (testMatrix && testMatrix.length > 0) {
        ...
        for (const tr of [...testMatrix].sort((a, b) => b.gmean - a.gmean)) {
            ...
            lines.push(`| ${tr.variantId.slice(0, 16)} | ${(tr.gmean * 100).toFixed(1)} | ` +
 ...
 */
function collectPairedDifferences(a, b) {
    const out = [];
