...
function findNearestRejectedReset(windows) {
    let nearest;
...
function getWindowWaitMs(rl) {
    const nearest = findNearestRejectedReset(rl.windows);
    if (nearest)
        return Math.max(10_000, nearest - Date.now());
    if (rl.resetsAt && rl.resetsAt > Date.now())
        return Math.max(10_000, rl.resetsAt - Date.now());
    if (rl.utilization >= 0.9)
        return 60_000;
    if (rl.utilization >= 0.75)
        return 15_000;
    return 0;
}
function checkShouldWait(rl) {
    const nearest = findNearestRejectedReset(rl.windows);
