...
describe("batch discrimination (sanity gate)", () => {
    ...
    it("batch cost reports half the online cost (approximate)", async () => {
        ...
        const rows = await buildMatrix([{ id: "default", promptPath: "test/plan", generation: 0, text: "SENTINEL budget=5" }], [makeCase("c1", 5)], { model: "mock", batch: true, batchCallModel: mockBatch });
    ...
    it("handles reps correctly under batch (every rep submitted with unique custom_id)", async () => {
        const customIdsSeen = new Set();
        const mockBatch = async (jobs) => {
            for (const j of jobs)
                customIdsSeen.add(j.customId);
            const out = new Map();
            for (const j of jobs) {
        ...
        const rows = await buildMatrix([{ id: "default", promptPath: "test/plan", generation: 0, text: "SENTINEL budget=5" }], [makeCase("c1", 5)], { model: "mock", batch: true, repetitions: 4, batchCallModel: mockBatch });
