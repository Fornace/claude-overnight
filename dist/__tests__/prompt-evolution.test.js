...
describe("scorer", () => {
    ...
    it("computes geometric mean", () => {
        const scores = { parse: 1, schema: 1, content: 1, costEfficiency: 1, speed: 1 };
        assert.equal(gmean(scores), 1);
        const half = { parse: 0.5, schema: 0.5, content: 0.5, costEfficiency: 0.5, speed: 0.5 };
        assert.equal(gmean(half), 0.5);
    });
});
describe("curator", () => {
    function makeRow(id, g, scores = {}) {
        const s = {
