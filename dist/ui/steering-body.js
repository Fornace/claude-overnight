...
function lastWaveRows(w, lw) {
    const out = [divider(w, `Wave ${lw.wave + 1} summary`)];
...
function statusRows(w, status) {
    const lines = status.trim().split("\n").filter(l => l.trim()).slice(0, 6);
    if (lines.length === 0)
        return [];
    const out = [divider(w, "Status")];
    const indent = "  ";
...
function plannerRows(w, events, startedAt, plannerModel) {
    const plannerModelTag = plannerModel ? chalk.dim(` \u00b7 ${modelDisplayName(plannerModel)}`) : "";
    const out = [divider(w, `Planner activity${plannerModelTag}`)];
...
export function SteeringBody({ runInfo, context, events, startedAt, statusLine, rlGetter }) {
