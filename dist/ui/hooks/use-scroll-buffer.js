...
export function useScrollBuffer(items, visibleCount) {
    ...
    const clamp = useCallback((offset) => Math.max(0, Math.min(offset, Math.max(0, items.length - visibleCount))), [items.length, visibleCount]);
    ...
    const viewportItems = useMemo(() => items.slice(scrollOffset, scrollOffset + visibleCount), [items, scrollOffset, visibleCount]);
