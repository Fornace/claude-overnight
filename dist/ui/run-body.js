...
function taskColumnWidth(w) {
    return Math.max(20, Math.min(36, w - 50 - COL_MODEL_W - 6));
}
function fmtRow(a, w, selected, fallbackModel) {
...
function agentTable(swarm, selectedAgentId) {
    ...
    for (const a of show)
        rows.push(fmtRow(a, w, a.id === (selectedAgentId ?? -1), swarm.model));
    if (swarm.pending > 0)
        rows.push(chalk.gray(`  ... + ${swarm.pending} queued`));
    return rows;
}
function detailRows(swarm, id) {
    ...
    if (agent.currentTool)
        meta.push(chalk.yellow(`tool: ${agent.currentTool}`));
    if (agent.lastText)
        meta.push(chalk.dim(truncate(agent.lastText, 60)));
    if (agent.filesChanged != null)
        meta.push(chalk.dim(`${agent.filesChanged} files`));
    if (agent.costUsd != null)
        meta.push(chalk.yellow(`$${agent.costUsd.toFixed(3)}`));
    if (agent.toolCalls > 0)
        meta.push(chalk.dim(`${agent.toolCalls} tools`));
...
function mergeRows(swarm) {
    if (swarm.mergeResults.length === 0)
        return [];
    const w = terminalWidth();
    const rows = [divider(w, "Merges")];
...
function eventRows(swarm) {
    const w = terminalWidth();
    const rows = [divider(w, "Events")];
