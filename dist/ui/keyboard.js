...
export function handlePaste(host, state, text) {
    ...
    if (state.mode === "steer" || state.mode === "ask") {
        if (segmentsToString(state.segs).length + text.length > MAX_INPUT_LEN)
            return false;
        appendPasteToSegments(state.segs, text);
        return true;
    }
...
export function handlePanelKey(host, s) {
    const panel = host.panel;
    const bodyRows = Math.max(3, (process.stdout.rows || 40) - 7);
    // CSI sequences: arrows, PgUp/PgDn, Home/End
    if (s.startsWith("\x1B[")) {
        if (s === "\x1B[A") {
            panel.scroll("up", bodyRows);
            return true;
        }
        if (s === "\x1B[B") {
            panel.scroll("down", bodyRows);
            return true;
        }
        if (s === "\x1B[5~") {
            panel.pageScroll("up", bodyRows);
            return true;
        }
        if (s === "\x1B[6~") {
            panel.pageScroll("down", bodyRows);
            return true;
        }
        if (s === "\x1B[H" || s === "\x1B[1~") {
            panel.scrollToTop();
    ...
    if (s === "\x03") {
        if (host.swarm && !host.swarm.aborted) {
            host.swarm.abort();
            return true;
        }
        process.exit(0);
 ...
 */
export function handleTyped(host, state, s) {
    ...
    if (s === "\t" && state.mode === "settings") {
        ...
        state.settingsField++;
        state.segs = [];
        if (state.settingsField >= SETTINGS_FIELDS.length)
            state.mode = "none";
        return true;
    }
    // ── 4. Settings mode ──
    ...
    if (s === "\r" || s === "\n") {
        if (host.hasAskTempFile)
            host.openAskTempFile();
        return true;
    }
    // Ctrl+C
    if (s === "\x03") {
...
export function renderInputPrompt(host, state) {
    if (state.mode === "none")
        return "";
...
export function bindKeyboard(host, state, onDirty) {
    const listener = (buf) => {
        const chunk = buf.toString();
        let dirty = false;
        for (const seg of splitPaste(chunk)) {
            if (seg.type === "paste") {
                if (handlePaste(host, state, seg.text))
                    dirty = true;
            }
            else {
                if (handleTyped(host, state, seg.text))
                    dirty = true;
            }
        }
        if (dirty)
