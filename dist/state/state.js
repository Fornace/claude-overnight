...
export function writeGoalUpdate(baseDir, update) {
...
export function loadRunState(runDir) {
    const state = readJsonOrNull(join(runDir, "run.json"));
    if (!state)
        return null;
    if (!Array.isArray(state.branches))
        state.branches = [];
    if (!Array.isArray(state.currentTasks))
        state.currentTasks = [];
    return state;
}
/** List run dir names. Newest-first when `newestFirst`, otherwise insertion order. Missing → []. */
function listRunDirs(rootDir, newestFirst = true) {
    try {
...
export function findIncompleteRuns(rootDir, filterCwd) {
...
export function findOrphanedDesigns(rootDir) {
    const runsDir = join(rootDir, "runs");
    for (const d of listRunDirs(rootDir)) {
        const runDir = join(runsDir, d);
        if (existsSync(join(runDir, "run.json")))
            continue;
        if (readMdDir(join(runDir, "designs")))
            return runDir;
    }
    return null;
}
...
export function backfillOrphanedPlans(rootDir, filterCwd) {
    ...
    for (const d of listRunDirs(rootDir, false)) {
        ...
        if (existsSync(join(runDir, "run.json")))
            continue;
        const parsed = readJsonOrNull(join(runDir, "tasks.json"));
...
export function createRunDir(rootDir) {
    const ts = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
...
export function updateLatestSymlink(rootDir, runDir) {
...
export function saveWaveSession(baseDir, waveNum, agents, totalCost) {
...
export function loadWaveHistory(runDir) {
    const dir = join(runDir, "sessions");
    let names;
    try {
        names = readdirSync(dir).filter(f => f.startsWith("wave-") && f.endsWith(".json"));
    }
    catch {
        return [];
    }
...
export function archiveMilestone(baseDir, waveNum) {
