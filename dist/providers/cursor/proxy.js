...
import { PROXY_DEFAULT_URL, cursorProxyOutLogPath, cursorProxyFetchOpts, resolveAgentPaths, resolveCursorComposerCli, getEmbeddedComposerProxyVersion, resolveCursorAgentToken, resolveCursorProxyKey, ensureCursorAccountPool, warnMacCursorAgentShellPatchIfNeeded, } from "./env.js";
 ...
 */
async function probeProxyHealth(baseUrl = PROXY_DEFAULT_URL) {
    const url = baseUrl.replace(/\/$/, "");
    const fetchOpts = { method: "GET", signal: AbortSignal.timeout(3_000), ...cursorProxyFetchOpts() };
    try {
...
async function startProxyProcess(baseUrl, _url, port) {
 ...
 */
export async function preflightCursorProxyViaHttp(p, timeoutMs, opts) {
...
async function postProxyMessages(baseURL, headers, model, userContent, timeoutMs, opts, progressLabel) {
 ...
 */
async function probeCursorWriteCapability(baseURL, headers, model, timeoutMs, opts) {
