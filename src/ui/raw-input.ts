...
export type InputEvent =
  ...
  | { type: "interrupt" }
  | { type: "tab" }
  | { type: "nav"; name: "up" | "down" | "left" | "right" | "home" | "end" | "pgup" | "pgdn" };
...
function csiToNav(body: string): InputEvent | null {
  ...
  if (body === "A") return { type: "nav", name: "up" };
  if (body === "B") return { type: "nav", name: "down" };
  if (body === "C") return { type: "nav", name: "right" };
  if (body === "D") return { type: "nav", name: "left" };
  if (body === "H" || body === "1~" || body === "7~") return { type: "nav", name: "home" };
  if (body === "F" || body === "4~" || body === "8~") return { type: "nav", name: "end" };
  if (body === "5~") return { type: "nav", name: "pgup" };
  if (body === "6~") return { type: "nav", name: "pgdn" };
  return null;
}
...
export function parseChunk(chunk: string): InputEvent[] {
  ...
  const parts: Array<{ kind: "free" | "paste"; text: string }> = [];
  let i = 0;
  while (i < chunk.length) {
    const ps = chunk.indexOf(PASTE_START, i);
    if (ps === -1) { parts.push({ kind: "free", text: chunk.slice(i) }); break; }
    if (ps > i) parts.push({ kind: "free", text: chunk.slice(i, ps) });
    const bodyStart = ps + PASTE_START.length;
    const pe = chunk.indexOf(PASTE_END, bodyStart);
    if (pe === -1) { parts.push({ kind: "paste", text: chunk.slice(bodyStart) }); break; }
  ...
  for (const part of parts) {
    if (part.kind === "paste") {
      if (part.text) out.push({ type: "paste", text: part.text });
      continue;
    }
    ...
    if (s === "\r" || s === "\n" || s === "\r\n") { out.push({ type: "submit" }); continue; }
    if (s === "\x03") { out.push({ type: "interrupt" }); continue; }
    if (s === "\x7F" || s === "\b") { out.push({ type: "backspace" }); continue; }
    if (s === "\t") { out.push({ type: "tab" }); continue; }
    if (s === "\x1B") { out.push({ type: "cancel" }); continue; }
    if (s === "\x15") { out.push({ type: "clear-line" }); continue; } // ^U
    if (s === "\x17") { out.push({ type: "word-delete" }); continue; } // ^W
    ...
    if ((s.includes("\n") || s.includes("\r")) && s.length > 2) {
      ...
      if (stripped) out.push({ type: "paste", text: stripped });
      continue;
    }
    ...
    let j = 0;
    let buf = "";
    const flushBuf = () => { if (buf) { out.push({ type: "char", text: buf }); buf = ""; } };
    ...
    while (j < s.length) {
      ...
      if (ch === "\x03") { flushBuf(); out.push({ type: "interrupt" }); j++; continue; }
      if (ch === "\x7F" || ch === "\b") { flushBuf(); out.push({ type: "backspace" }); j++; continue; }
      if (ch === "\t") { flushBuf(); out.push({ type: "tab" }); j++; continue; }
      if (ch === "\x15") { flushBuf(); out.push({ type: "clear-line" }); j++; continue; }
      if (ch === "\x17") { flushBuf(); out.push({ type: "word-delete" }); j++; continue; }

      if (ch === "\x1B") {
        flushBuf();
        const next = j + 1 < s.length ? s[j + 1] : null;
        if (next === null) { out.push({ type: "cancel" }); j++; continue; }
      ...
      if (code < 0x20 || (code >= 0x7F && code < 0xA0)) { j++; continue; }
...
export function setBracketedPaste(stdout: NodeJS.WriteStream, enabled: boolean): void {
  try { stdout.write(enabled ? "\x1B[?2004h" : "\x1B[?2004l"); } catch {}
}
