...
describe("Swarm.active", () => {
  ...
  it("counts only agents whose status is 'running'", () => {
    const swarm = makeSwarm();
    swarm.agents.push(
      { id: 0, task: makeTasks(1)[0], status: "running", toolCalls: 0 },
      { id: 1, task: makeTasks(1)[0], status: "done", toolCalls: 0 },
      { id: 2, task: makeTasks(1)[0], status: "running", toolCalls: 0 },
      { id: 3, task: makeTasks(1)[0], status: "error", toolCalls: 0 },
    );
  ...
  it("returns 0 when all agents are done or errored", () => {
    const swarm = makeSwarm();
    swarm.agents.push(
      { id: 0, task: makeTasks(1)[0], status: "done", toolCalls: 3 },
      { id: 1, task: makeTasks(1)[0], status: "error", toolCalls: 1 },
    );
