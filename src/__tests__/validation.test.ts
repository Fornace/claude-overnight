...
// its validation here to unit-test the logic in isolation.
  ...
  describe("missing tasks key", () => {
    it("throws when object has no tasks property", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /must contain a "tasks" array/ },
        );
      } finally {
        teardown();
      }
    ...
    it("throws when tasks is not an array", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /must contain a "tasks" array \(got string\)/ },
        );
      } finally {
        teardown();
      }
  ...
  describe("task with empty string", () => {
    it("throws on empty string task", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Task 1 is an empty string/ },
        );
      } finally {
        teardown();
      }
    ...
    it("throws on whitespace-only string task", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Task 0 is an empty string/ },
        );
      } finally {
        teardown();
      }
  ...
  describe("task object missing prompt", () => {
    it("throws when prompt key is absent", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Task 0 is missing a "prompt" string/ },
        );
      } finally {
        teardown();
      }
  ...
  describe("unknown keys", () => {
    it("throws on a single unknown key", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Unknown key in task file: foo/ },
        );
      } finally {
        teardown();
      }
    ...
    it("throws on multiple unknown keys with plural message", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Unknown keys in task file: foo, bar/ },
        );
      } finally {
        teardown();
      }
  ...
  describe("invalid concurrency", () => {
    it("throws on zero", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Concurrency must be a positive integer \(got 0\)/ },
        );
      } finally {
        teardown();
      }
    ...
    it("throws on negative number", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Concurrency must be a positive integer \(got -3\)/ },
        );
      } finally {
        teardown();
      }
    ...
    it("throws on float", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Concurrency must be a positive integer \(got 2\.5\)/ },
        );
      } finally {
        teardown();
      }
    ...
    it("throws on string", () => {
      ...
      try {
        ...
        assert.throws(
          () => loadTaskFile(file),
          { message: /Concurrency must be a positive integer \(got "4"\)/ },
        );
      } finally {
        teardown();
      }
