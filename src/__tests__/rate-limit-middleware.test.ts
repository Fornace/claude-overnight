...
import type { RateLimitInfo } from "../middleware/rate-limit.js";

type TestReq = { remoteAddress: string; headers: Record<string, string> };

function makeReq(overrides: { remoteAddress?: string; headers?: Record<string, string> } = {}): TestReq {
  return {
...
function run(mw: ReturnType<typeof rateLimit>, req: TestReq) {
  let result: { status: number; headers: Record<string, string>; body: string } | undefined;
  mw(req, (r) => { if (r) result = r; });
  return result;
}
...
describe("rateLimit middleware", () => {
  ...
  it("custom message as function receiving info", () => {
    const mw = rateLimit({
      maxRequests: 1,
      windowMs: 60000,
      message: (info: RateLimitInfo) => `limited: ${info.current}/${info.limit}`,
    });
    run(mw, makeReq());
    const result = run(mw, makeReq());
    assert.ok(result);
  ...
  it("falls back to header-based key when no remoteAddress", () => {
    ...
    (req as any).remoteAddress = undefined;
    run(mw, req);
