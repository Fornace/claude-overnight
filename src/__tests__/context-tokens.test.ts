...
import type { AgentState, Task } from "../core/types.js";
...
describe("context token accumulation", () => {
  it("tracks the latest turn total (input + cache) across assistant messages", () => {
    ...
    handleMsg(swarm as any, agent, fakeAssistant(1000, 500, 0) as any);
    ...
    handleMsg(swarm as any, agent, fakeAssistant(800, 200, 0) as any);
    ...
    handleMsg(swarm as any, agent, fakeAssistant(2000, 1000, 100) as any);
  ...
  it("is resilient to missing usage", () => {
    ...
    const agent = makeAgent();
    (swarm as any).agents.push(agent);
    handleMsg(swarm as any, agent, { type: "assistant", message: { content: [] } } as any);
  ...
  it("logs a saturation warning when crossing 80% of safeContext", () => {
    ...
    handleMsg(swarm as any, agent, fakeAssistant(50_000, 0, 0) as any);
    ...
    handleMsg(swarm as any, agent, fakeAssistant(55_000, 0, 0) as any);
