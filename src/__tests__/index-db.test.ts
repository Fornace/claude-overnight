...
describe("index-db — FTS5", () => {
  it("FTS5 triggers populate on insert", () => {
    ...
    const results = db.prepare("SELECT rowid, name, rank FROM skills_fts WHERE skills_fts MATCH ? ORDER BY rank LIMIT 5").all("search") as any[];
...
describe("index-db — events + incrementUse", () => {
  it("recordEvent inserts a row", () => {
    ...
    const db = openSkillsDb();
    const count = (db.prepare("SELECT COUNT(*) as c FROM skill_events WHERE skill_name = ?").get("my-skill") as any).c;
  ...
  it("incrementUse bumps uses and last_used_at", () => {
    ...
    const row = db.prepare("SELECT uses, last_used_at FROM skills WHERE name = ?").get("inc-skill") as any;
