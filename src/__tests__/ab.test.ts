...
function seedSkill(db: ReturnType<typeof openSkillsDb>, name: string, fp: string, abEligible = 1, quarantined = 0) {
  db.prepare(
    `INSERT INTO skills(name, repo_fingerprint, description, version, languages, toolsets, requires_tools, triggers, body_path, size_bytes, created_at, ab_eligible, quarantined)
     VALUES (?, ?, ?, 1, '[]', '[]', '[]', '[]', '/dev/null', 0, ?, ?, ?)`,
  ).run(name, fp, `desc ${name}`, NOW, abEligible, quarantined);
}
...
describe("ab — pickAbSkill", () => {
  ...
  it("returns null when fewer than 2 tasks", () => {
    ...
    const tasks: Task[] = [{ id: "a", prompt: "only one" }];
...
function mkAssignment(skill: string): AbAssignment {
  return { skill, treatmentTaskIds: ["t1"], controlTaskIds: ["c1"], wave: 0 };
}

function recordRows(skill: string) {
  const db = openSkillsDb();
  return db.prepare("SELECT * FROM skill_events WHERE skill_name = ? ORDER BY rowid").all(skill) as { event: string; value: number; notes: string }[];
}
