...
interface Captured { url: string; headers: Record<string, string | string[] | undefined>; body: any; }
...
async function withMockServer(
  ...
  const server: Server = createServer((req: IncomingMessage, res: ServerResponse) => {
    ...
    req.on("end", () => {
      const cap: Captured = { url: req.url ?? "", headers: req.headers, body: JSON.parse(raw) };
      captured.push(cap);
  ...
  try { await fn(port, captured); }
  finally { await new Promise<void>((r) => server.close(() => r())); }
}

function seedCandidate(body: string = "# heuristic\nuse this skill when the task involves X"): void {
...
describe("librarian fetch — direct POST to /v1/messages", () => {
  it("POSTs to proxied base url with Bearer and applies returned actions", async () => {
    ...
    await withMockServer(
      () => ({ status: 200, body: { content: [{ type: "text", text: JSON.stringify(actions) }] } }),
      async (port, captured) => {
        ...
        const row = db.prepare("SELECT name, description FROM skills WHERE name = ?").get("new-skill-1") as any;
  ...
  it("strips markdown code fences around JSON response", async () => {
    ...
    await withMockServer(
      () => ({ status: 200, body: { content: [{ type: "text", text: fenced }] } }),
      async (port) => {
        const result = await runLibrarian({
          fingerprint: FP, runId: "r1", wave: 1, cwd: tmp, model: "composer-2",
  ...
  it("returns empty result when response is not valid JSON", async () => {
    seedCandidate();
    await withMockServer(
      () => ({ status: 200, body: { content: [{ type: "text", text: "I am not JSON, sorry." }] } }),
      async (port) => {
        const result = await runLibrarian({
          fingerprint: FP, runId: "r1", wave: 1, cwd: tmp, model: "composer-2",
          envForModel: () => ({ ANTHROPIC_BASE_URL: `http://127.0.0.1:${port}`, ANTHROPIC_AUTH_TOKEN: "t" }),
