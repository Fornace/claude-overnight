...
import type { CallModel, CallModelResult } from "../prompt-evolution/transport.js";
 ...
 */
function mockCallModelFor(variantBehavior: Map<string, (budget: number) => string>): CallModel {
  return async (userText: string): Promise<CallModelResult> => {
...
describe("discrimination (sanity gate)", () => {
  ...
  it("ranks good above broken when broken returns invalid JSON", async () => {
    const behavior = new Map<string, (b: number) => string>([
  ...
  it("ranks good above broken when broken is wildly off-budget", async () => {
    ...
    const good = rows.find((r) => r.variantId === "good")!;
    const broken = rows.find((r) => r.variantId === "broken")!;
    assert.ok(good.aggregate.content > broken.aggregate.content + 0.3,
      `good content (${good.aggregate.content.toFixed(3)}) must beat broken content (${broken.aggregate.content.toFixed(3)}) by ≥0.3`);
  });

  it("reports cross-model stddev when multiple models are used", async () => {
    // Simulate one strong model, one weak model.
    const callModel: CallModel = async (userText, _sys, opts) => {
      const budgetMatch = userText.match(/budget=(\d+)/);
      const budget = budgetMatch ? parseInt(budgetMatch[1], 10) : 5;
    ...
    const rows = await buildMatrix(
      ...
      cases,
      { model: "strong-model", models: ["strong-model", "weak-model"], callModel },
    );
    const row = rows[0];
    assert.ok(row.crossModelStddev != null && row.crossModelStddev > 0.1,
      `expected cross-model stddev > 0.1 when one model fails, got ${row.crossModelStddev}`);
  ...
  it("aggregates repetitions into a single result with a stddev", async () => {
    ...
    const callModel: CallModel = async (userText) => {
      callCount++;
      const budgetMatch = userText.match(/budget=(\d+)/);
      const budget = budgetMatch ? parseInt(budgetMatch[1], 10) : 5;
    ...
    const rows = await buildMatrix(
      [{ id: "flaky", promptPath: "test/plan", generation: 0, text: "SENTINEL budget=5" }],
      [makeCase("c1", 5)],
      { model: "mock", repetitions: 4, callModel },
    );
