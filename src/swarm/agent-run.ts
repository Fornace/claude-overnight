...
import { withCursorWorkspaceHeader, getAgentTimeout, type SwarmConfig } from "./config.js";
...
export async function runAgent(host: AgentRunHost, task: Task): Promise<void> {
  ...
  const agent: AgentState = { id, task, status: "running", startedAt: Date.now(), toolCalls: 0, contextTokens: 0, model: task.model || host.model };
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (resumeSessionId && agent.status === "running") {
        try { host.log(id, "Simplify pass"); resumePrompt = renderPrompt("20_execution/20-1_simplify"); await runTurn(true, resumePrompt); }
      ...
      if (agent.status === "running") {
        ...
        if (agent.toolCalls === 0 && (agent.costUsd ?? 0) < 0.001 && duration < 15_000) {
          agent.status = "error"; agent.error = "Agent did no work  -- exited without tool use"; host.failed++;
      ...
      if (canRetry) { host.log(id, `Transient error: ${String(err.message || err).slice(0, 80)}`); continue; }
      agent.status = "error"; agent.error = String(err.message || err).slice(0, 120);
      agent.finishedAt = Date.now(); host.failed++;
      host.log(id, agent.error);
    }
  }
...
function hasLspMcp(): boolean {
  ...
  for (const p of candidates) {
    if (/\b(cclsp|serena)\b/.test(readFileOrEmpty(p))) { _lspMcpPresent = true; return true; }
  }
