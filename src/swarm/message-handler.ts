...
export function logToolUse(host: MessageHandlerHost, agent: AgentState, name: string, input: Record<string, unknown>): void {
  const target = extractToolTarget(input);
  host.log(agent.id, target ? `${name} \u2192 ${target}` : name);
}

export function handleMsg(host: MessageHandlerHost, agent: AgentState, msg: SDKMessage): void {
  ...
  switch (msg.type) {
    ...
    case "result": {
      ...
      agent.costUsd = cost; host.totalCostUsd += cost;
      if (host.isUsingOverage) host.overageCostUsd += cost;
      if (r.usage) {
      ...
      if (r.subtype === "success") {
        ...
        if (Array.isArray(errs) && errs.length > 0) {
          parts.push(errs[0]);
          for (const e of errs.slice(1, 3)) host.log(agent.id, `err: ${String(e).slice(0, 160)}`);
        }
