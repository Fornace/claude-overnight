...
export function modelDisplayName(model: string): string {
  ...
  if (MODEL_CAPABILITIES[model]?.displayName) return MODEL_CAPABILITIES[model].displayName!;
  // Substring match
  for (const [key, cap] of Object.entries(MODEL_CAPABILITIES)) {
    if (key !== "unknown" && m.includes(key) && cap.displayName) return cap.displayName!;
  }
  return model;
}
...
export function formatContextWindow(model: string): string {
  const cap = getModelCapability(model);
  if (cap.contextWindow >= 1_000_000) return `${(cap.contextWindow / 1_000_000).toFixed(1)}M`;
