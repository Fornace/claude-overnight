...
export async function generateFinalNarrative(deps: FinalNarrativeDeps, phase: string): Promise<string> {
  ...
  const cap = (s: string | undefined, n: number) => s ? truncate(s, n) : "";
...
export async function printFinalSummary(args: SummaryArgs): Promise<void> {
  const {
  ...
  if (waveHistory.length > 0) {
    ...
    for (const w of waveHistory) {
      ...
      const parts: string[] = [];
      if (doneCt) parts.push(chalk.green(`✓ ${doneCt}`));
