  ...
  while (runAnotherRound) {
    ...
    while (host.remaining > 0 && host.currentTasks.length > 0 && !ctx.isStopping()) {
      ...
      await throttleBeforeWave(
        ctx.rlGetter,
        (text) => ctx.display.appendSteeringEvent(text),
        ctx.isStopping,
      );
      ...
      if (ctx.beforeWaveCmds) {
        ...
        for (const cmd of cmds) {
          ...
          try {
            ...
            if (out.trim()) ctx.display.appendSteeringEvent(`  → ${out.trim().slice(0, 200)}`);
          } catch (err: any) {
      ...
      try { await swarm.run(); }
        ...
        if (isJWTAuthError(err)) { ctx.display.stop(); console.error(chalk.red(`\n  Authentication failed  -- check your API key or run: claude auth\n`)); process.exit(1); }
        if (swarm.agents.length > 0) {
          try { saveWaveSession(ctx.runDir, host.waveNum, swarm.agents, swarm.totalCostUsd); } catch {}
        }
        throw err;
      }
      ...
      // ── Stats rollup ──
      host.accCost += swarm.totalCostUsd; host.accIn += swarm.totalInputTokens; host.accOut += swarm.totalOutputTokens;
      host.accCompleted += swarm.completed; host.accFailed += swarm.failed;
      ...
      for (const a of swarm.agents) {
        ...
        if (pct > host.peakWorkerCtxPct) { host.peakWorkerCtxPct = pct; host.peakWorkerCtxTokens = tok; }
      }
      ...
      host.lastCapped = swarm.cappedOut; host.lastAborted = swarm.aborted;
      ...
      try {
        const unresolved = host.branches.filter(b => {
          ...
          try { execSync(`git rev-parse --verify "${b.branch}"`, { cwd: ctx.cwd, stdio: "ignore" }); return true; }
          catch { return false; }
        });
        ...
        const gcCandidates = host.branches.filter(b =>
          b.status === "merge-failed" && b.firstFailedWave !== undefined && (host.waveNum - b.firstFailedWave) >= 2
        );
        let gcCount = 0;
        for (const b of gcCandidates) {
          try { execSync(`git branch -D "${b.branch}"`, { cwd: ctx.cwd, stdio: "ignore" }); } catch {}
          b.status = "discarded";
          gcCount++;
        }
        if (gcCount > 0) ctx.display.appendSteeringEvent(`GC: discarded ${gcCount} ghost branch(es) ≥2 waves old`);
      } catch {}

      // Fast-exit on user-quit: don't spend more budget on debrief / after-wave
      // / post-wave review — the user wants to stop NOW.
      if (ctx.isStopping() || swarm.aborted) break;
    ...
    const exhaustedByBudget =
      !host.objectiveComplete && !ctx.isStopping() && !host.lastAborted && !host.lastCapped &&
      host.remaining <= 0;
...
function handleZeroWorkRetry(swarm: Swarm, host: WaveLoopHost, ctx: WaveLoopCtx): void {
  ...
  ctx.display.resume();
  try { retrySwarm.run(); } catch {}
  ctx.display.pause();

  host.accIn += retrySwarm.totalInputTokens; host.accOut += retrySwarm.totalOutputTokens;
  ...
  const stillZero = retrySwarm.agents.filter(a => {
    ...
    try { execSync(a.task.postcondition, { cwd: ctx.cwd, stdio: "ignore", timeout: 30_000 }); return false; }
    catch { return true; }
  });
