...
import type { Task, PermMode, RateLimitWindow } from "../core/types.js";
...
export function postProcess(raw: Task[], budget: number | undefined, onLog: (text: string) => void): Task[] {
  ...
  if (tasks.length < before) onLog(`Filtered ${before - tasks.length} task(s) with empty prompt`);
  ...
  if ((budget ?? 10) <= 15) {
    ...
    for (let i = 0; i < tasks.length; i++) adj.set(i, new Set());
    const pathToIndices = new Map<string, number[]>();
    for (let i = 0; i < tasks.length; i++) {
      const t = tasks[i];
      if (t.type && t.type !== "execute") continue;
      for (const m of t.prompt.matchAll(fileRe)) {
        const indices = pathToIndices.get(m[1]);
        if (indices) {
          for (const j of indices) { adj.get(i)!.add(j); adj.get(j)!.add(i); }
          indices.push(i);
        } else {
    ...
    let totalMerged = 0;
    for (let i = 0; i < tasks.length; i++) {
      if (visited.has(i) || adj.get(i)!.size === 0) continue;
      ...
      while (stack.length > 0) {
        ...
        visited.add(curr);
        component.push(curr);
        for (const nb of adj.get(curr)!) if (!visited.has(nb)) stack.push(nb);
      }
      if (component.length > 1) {
        ...
        for (const idx of component.slice(1)) tasks.splice(idx, 1);
        tasks[component[0]] = merged;
        totalMerged += component.length - 1;
      }
    }
    if (totalMerged > 0) onLog(`Merged ${totalMerged} overlapping task(s) into combined tasks`);
  }
...
export function attemptJsonParse(text: string): any | null {
  ...
  if (preface.test(text) || suffix.test(text)) {
    ...
    if (cleaned && cleaned !== text) {
      try { const obj = JSON.parse(cleaned); if (typeof obj === "object" && obj !== null) return obj; } catch {}
    }
  }
