...
import type {
  SDKMessage, SDKAssistantMessage, SDKPartialAssistantMessage,
  SDKRateLimitEvent, SDKResultMessage,
...
import {
  type PlannerLog, addPlannerCost, recordPeakContext, setContextTokens,
  applyRateLimitEvent, getPlannerRateLimitInfo,
} from "./throttle.js";
import { sdkQueryRateLimiter, acquireSdkQueryRateLimit } from "../core/rate-limiter.js";
import { StallGuard, runWithStallRotation, StallMonitor, type StallGuardSink } from "../core/stall-guard.js";
...
import type { StreamEvent, AssistantUsage, RateLimitInfoFragment } from "./sdk-events.js";
...
export async function runPlannerStreamWithRotation(
  prompt: string, opts: PlannerOpts, onLog: PlannerLog,
  initialEnv: Record<string, string> | undefined,
  isResume: boolean, tname: string | undefined,
): Promise<string> {
...
async function runOneStream(
  ...
  isResume: boolean, startedAt: number, tname: string | undefined,
): Promise<string> {
  ...
  let pendingTool: { name: string; id: string; input: Record<string, unknown>; buf: string; logged: boolean } | null = null;
  ...
  const stallSink: StallGuardSink = { lastByteAt: Date.now(), streamId: "", finished: false };
  const stallGuard = new StallGuard(stallSink, new AbortController());
  const stallPromise = new Promise<never>((_, reject) => { stallGuard.on("stall", reject); });
  ...
  const handleStreamEvent = (msg: SDKPartialAssistantMessage): void => {
  ...
  const handleAssistant = (msg: SDKAssistantMessage): void => {
    const usage = msg.message?.usage as AssistantUsage | undefined;
    if (usage) {
      ...
      if (opts.turnId) {
        ...
        if (turn) {
          const totalCost = (msg as { total_cost_usd?: unknown }).total_cost_usd;
          updateTurn(turn, {
    ...
    for (const part of content as unknown as Array<Record<string, unknown>>) {
      ...
      if (t === "tool_use" && typeof part.id === "string" && !seenToolIds.has(part.id)) {
        ...
        logTool(name, input);
        if (tname) writeTranscriptEvent(tname, { kind: "tool_use", tool: name, input });
  ...
  const handleRateLimit = (msg: SDKRateLimitEvent): void => {
    const info = msg.rate_limit_info as RateLimitInfoFragment | undefined;
    if (!info) return;
  ...
  const handleResult = (msg: SDKResultMessage): void => {
    ...
    if (tname) writeTranscriptEvent(tname, {
      ...
      costUsd, durationMs: Date.now() - startedAt, toolCount,
      error: (msg as { errors?: string[] }).errors?.join("; "),
    });
    throw new Error(`Planner failed: ${msg.subtype}`);
  };

  const consume = async (): Promise<void> => {
    for await (const msg of pq as AsyncIterable<SDKMessage>) {
      lastActivity = Date.now();
      stallSink.lastByteAt = Date.now();
      msgCount++;
      if (!sessionId && "session_id" in msg) sessionId = msg.session_id;
      switch (msg.type) {
        case "stream_event":     handleStreamEvent(msg); break;
        case "assistant":        handleAssistant(msg); break;
        case "rate_limit_event": handleRateLimit(msg); break;
...
function extractSkillProposal(text: string): { trigger: string; body: string } | undefined {
  const m = text.match(/###\s*SKILL CANDIDATE\s*\n([\s\S]+?)$/);
  if (!m) return undefined;
  const block = m[1].trim();
  const triggerM = block.match(/^trigger:\s*(.+)$/m);
  if (!triggerM) return undefined;
