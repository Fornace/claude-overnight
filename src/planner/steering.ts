...
function renderSteer(args: {
  objective: string; history: WaveSummary[]; remainingBudget: number;
  workerModel: string; fastModel: string | undefined; concurrency: number;
  runMemory: RunMemory | undefined; keepLastWaves: number; caps: SteerCaps;
}): string {
  const cap = (s: string | undefined, max: number): string =>
...
export async function steerWave(
  ...
  let caps: SteerCaps = { ...DEFAULT_CAPS };
  let keepLastWaves = 3;
  ...
  for (const step of trimSteps) {
    if (prompt.length <= PROMPT_BUDGET) break;
    step();
    const next = renderSteer({ ...base, caps, keepLastWaves });
    if (next.length < prompt.length) { prompt = next; trimmed++; }
  }
  if (trimmed > 0) {
  ...
  const parsed = await (async () => {
    ...
    const retryText = await runPlannerQuery(
      renderPrompt("30_wave/30-7_steer-retry", { vars: { snippet } }),
