...
export function autoMergeBranches(cwd: string, branches: BranchRecord[], onLog: (msg: string) => void): void {
  ...
  const tryGit = (cmd: string): boolean => { try { execSync(cmd, opts); return true; } catch { return false; } };
  onLog(`Merging ${unmerged.length} unmerged branches...`);
  for (const br of unmerged) {
    if (tryGit(`git merge --no-edit "${br.branch}"`)) {
      br.status = "merged"; onLog(`  ✓ ${br.branch} (${br.filesChanged} files)`); continue;
    }
    tryGit("git merge --abort");
    if (tryGit(`git merge --no-edit -X theirs "${br.branch}"`)) {
      br.status = "merged"; onLog(`  ✓ ${br.branch} (auto-resolved)`); continue;
    }
    tryGit("git merge --abort");
    if (forceMergeOverlay(br.branch, cwd)) {
      br.status = "merged"; onLog(`  ✓ ${br.branch} (force-merged)`);
    } else {
      br.status = "merge-failed"; onLog(`  ✗ ${br.branch} (conflict  -- preserved for manual merge)`);
    }
  }
}
