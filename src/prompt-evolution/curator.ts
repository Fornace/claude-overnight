...
import type { VariantRow, CuratorDecision, ScoreDimensions } from "./types.js";
...
export function curate(
  ...
  if (rows.length === 0) return { promoted: [], quarantined: [], kept: [] };
  ...
  for (const a of withNovelty) {
    ...
    if (!dominated) paretoIds.add(a.variantId);
  }
...
function dominates(a: ScoreDimensions, b: ScoreDimensions): boolean {
  const keys = ["parse", "schema", "content", "costEfficiency", "speed"] as const;
...
function vectorize(s: ScoreDimensions): number[] {
...
export function formatMatrix(rows: VariantRow[], caseNames: string[]): string {
  const lines: string[] = [];
  ...
  for (const r of rows.sort((a, b) => b.gmean - a.gmean)) {
    const s = r.aggregate;
    lines.push(
      `| ${r.variantId.slice(0, 12).padEnd(11)} | ${String(r.generation).padStart(3)} | ` +
...
function gmean(scores: ScoreDimensions): number {
