...
function renderBullets(items: Item[], header: string[], tokenBudget: number): { text: string; count: number; remaining: number } {
...
export function buildL0Stub(opts: { fingerprint: string; role?: string; tools?: string[]; excludeSkill?: string }): { text: string; count: number; remaining: number } {
  ...
  if (skills.length === 0) return { text: "", count: 0, remaining: 0 };
...
export function buildRecipeStub(opts: { fingerprint: string; tools?: string[] }): { text: string; count: number } | null {
