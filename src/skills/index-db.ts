...
export function openSkillsDb(): Database.Database {
  if (_db) return _db;
  ...
  try { _db.exec("ALTER TABLE skills ADD COLUMN ab_eligible INTEGER NOT NULL DEFAULT 0"); } catch {}
  try { _db.exec("ALTER TABLE skills ADD COLUMN ab_last_trial_run TEXT"); } catch {}
  // Phase 4 tool recipes: kind column to distinguish skills from recipes
  try { _db.exec("ALTER TABLE skills ADD COLUMN kind TEXT NOT NULL DEFAULT 'skill'"); } catch {}
...
export function queryCandidateL0(
  fp: string,
...
export function recordEvent(
  runId: string, wave: number, skill: string, event: string, value?: number, notes?: string,
): void {
  const db = openSkillsDb();
  db.prepare(
    "INSERT INTO skill_events(ts, run_id, wave, skill_name, event, value, notes) VALUES (?, ?, ?, ?, ?, ?, ?)",
  ).run(new Date().toISOString(), runId, wave, skill, event, value ?? null, notes ?? null);
}

/** Increment use counter and refresh last_used_at. */
export function incrementUse(skillName: string): void {
  const db = openSkillsDb();
  db.prepare(
...
export function queryAbEligibleSkills(fp: string): SkillRow[] {
  const db = openSkillsDb();
  return db.prepare(
    "SELECT * FROM skills WHERE repo_fingerprint = ? AND quarantined = 0 AND ab_eligible = 1 ORDER BY uses ASC LIMIT 5",
  ).all(fp) as SkillRow[];
}

/** Mark a skill's last A/B trial timestamp. */
export function markAbTrial(skillName: string): void {
  const db = openSkillsDb();
  db.prepare(
    "UPDATE skills SET ab_last_trial_run = ? WHERE name = ?",
  ).run(new Date().toISOString(), skillName);
}
