...
export async function editRunSettings(options: EditSettingsOptions): Promise<MutableRunSettings> {
  ...
  const plannerPick = await pickModel(
    `${chalk.cyan("①")} Planner model ${chalk.dim("(thinking, steering  -- use your strongest)")}:`,
    models, options.defaults?.plannerModel ?? s.plannerModel,
  );
  ...
  if (!options.cliConcurrencySet) {
    ...
    if (!isNaN(parsed) && parsed >= 1) s.concurrency = parsed;
  }

  const coachCap = options.defaults?.usageCap;
  const usageCapItems = [
    { name: "Unlimited", value: undefined as number | undefined, hint: "full capacity, wait through rate limits" },
...
export function formatSettingsSummary(s: MutableRunSettings): string {
  const parts: string[] = [];
  if (s.fastModel) parts.push(`${modelDisplayName(s.plannerModel)} → ${modelDisplayName(s.workerModel)} + ${modelDisplayName(s.fastModel)}`);
  else if (s.workerModel !== s.plannerModel) parts.push(`${modelDisplayName(s.workerModel)} → ${modelDisplayName(s.plannerModel)}`);
