...
export function refreshToken(oldToken: string, providerId: string): TokenRecord | null {
  ...
  if (result.payload.exp - now > 60) return null;
 ...
 */
export function revokeProvider(providerId: string): void {
  const cached = getCachedToken(providerId);
  if (cached) revokeSession(cached.sessionId);
  const peeked = tryPeekAndRevoke(providerId);
  if (!peeked) clearTokenCache();
}

function tryPeekAndRevoke(providerId: string): boolean {
...
export function isJWTAuthError(err: unknown): boolean {
  ...
  for (const indicator of genericAuth) {
    if (lower.includes(indicator) && hasContext) return true;
  }
