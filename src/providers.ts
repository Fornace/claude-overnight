...
function getEmbeddedComposerProxyVersion(): string | null {
  try {
    const require = createRequire(import.meta.url);
...
function getClaudeOvernightInstallRoot(): string {
 ...
 */
async function preflightCursorProxyViaHttp(
  p: ProviderConfig,
  timeoutMs: number,
  opts?: { onProgress?: (msg: string) => void },
): Promise<{ ok: true } | { ok: false; error: string }> {
 ...
 */
function resolveCursorAgentToken(): string | null {
