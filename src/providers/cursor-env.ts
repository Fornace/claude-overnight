...
export function resolveAgentPaths(timeoutMs = 2_000): [string | null, string | null] {
...
export function cachedAgentPaths(): [string | null, string | null] {
...
export function resolveCursorComposerCli(): string | null {
  try {
...
export function getEmbeddedComposerProxyVersion(): string | null {
  try {
    const require = createRequire(import.meta.url);
...
function cursorProxyLogDir(): string {
  return join(homedir(), ".cursor-api-proxy");
}

export function cursorProxyOutLogPath(): string {
  return join(cursorProxyLogDir(), "proxy.out.log");
}

export function cursorProxySessionsLogPath(): string {
 ...
 */
export function resolveCursorAgentToken(): string | null {
...
export function hasCursorMacAgentZshPatch(): boolean {
  let combined = "";
  for (const f of [".zshrc", ".zprofile"] as const) {
    try {
 ...
 */
export async function fetchLiveCursorModels(): Promise<string[]> {
